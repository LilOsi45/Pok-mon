...
# System events carry no game and fixed routes — kept in sync with heartbeat.py
# and watchdog.py, the two senders that bit us by going nowhere.
SYSTEM_SENDERS: tuple[tuple[str, list[str]], ...] = (
    ("Tagesbericht (heartbeat)", ["system:heartbeat"]),
    ("Störungs-Alarm (watchdog)", ["system:alarm", "system:heartbeat", "news:*"]),
)
...
def render(notifiers: list[Notifier], watches: list[Watch], scans: list[ProductScan]) -> str:
    ...
    out: list[str] = ["Eingerichtete Kanäle:"]
    for n in notifiers:
